...
To prioritise which road segments should be targetted for "greening action" there are two different methods we can use. The secon method relise on us getting information from the CoM about exactly what type of "greening actions" they are capable of implementing. We could also ask them to identify any roads where they would consider this to be  

- *Road segment removal*
   To simulate the removal of the "barrier" at each road segment we can remove that segment and then run the connectivity calculation. We should then be able to quantify the change in connectivity for each road segment, for each animal group. Those which show the greatest improvement could be considered priorities
